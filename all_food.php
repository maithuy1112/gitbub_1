...
//下面的跳轉網址
$_SESSION["url"] = "index.php?method=all_food";
$selected = 1;
//用戶搜尋食物
if (array_key_exists('submitbutton', $_POST)) {
  ...
  if (empty($search)) {
    ...
    $selected = 0;
    $selectfood_sql  = "select * from food,fridge where food.Frij_ID = fridge.Frij_ID and Food_Name like '%$search%' and Food_RsvnQty != 0 order by Food_Exp";
  }
} else {
  $selectfood_sql  = "select * from food,fridge where food.Frij_ID = fridge.Frij_ID and Food_RsvnQty != 0 order by Food_Exp";
}
                  ...
                  <div>
                    ...
                    <div class="detail-box">
                      ...
                      <div class="d-flex justify-content-end ">
                        ...
                        style="margin-right: 20px;color: white;font-weight: bold;" onclick="javascript:location.href=
                    'index.php?method=food_insidefridge&Frij_ID=<?php echo $row['Frij_ID'] ?>'">
                          查看冰箱
                      </button>

                        <button type="button" class="btn btn-warning" 
                        onclick="javascript:location.href='index.php?method=reservation_food&Frij_Addr=<?php echo $row['Frij_Addr'] ?>&Food_ID=<?php echo $row['Food_ID'] ?>'" 
